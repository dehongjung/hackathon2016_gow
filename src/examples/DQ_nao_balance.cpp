...
//    std::cout << std::endl <<"End Effector Pose Reference" << std::endl << eff_pose_reference << std::endl;
    ...
    double translation_threshold = 1.e-3;
	...
	eff_translation_difference = ( eff_pose_current.translation() - eff_pose_reference.translation());
...
//	std::cout << std::endl <<"Error" << std::endl << eff_translation_difference.vec4().norm() << std::endl;
    if( (eff_translation_difference.vec8().norm() > translation_threshold))// ||
...
{
    ros::init(argc, argv, "nao_dual_quaternions_balance");
    ros::NodeHandle n;
    ROS_INFO("nao dual quaternions balance is running...");
    
    //ros::ServiceServer service = n.advertiseService("find_angles_using_dq", DQ_invKin);
    ros::ServiceServer service = n.advertiseService("balance_using_dq", DQ_invKin);
