...
{
    ros::init(argc, argv, "nao_dual_quaternions_arm");
    ros::NodeHandle node_handle_main;
    ROS_INFO("nao dual quaternions ARMS is running...");

    ros::ServiceServer service = node_handle_main.advertiseService("find_angles_using_dq", DQ_invKin);
...
//    std::cout << std::endl <<"End Effector Pose Reference" << std::endl << eff_pose_reference_nao << std::endl;
    ...
    //Gain Matrix - posso melhorar bastante isso! =p
    ...
    DQ_kinematics nao_chain = NaoKinematics_Arm();
    ...
    if (chain == "LArm")
        ...
        {
        eff_pose_reference_nao[0] = eff_pose_reference_nao[0] ;//- 0.05775;
        eff_pose_reference_nao[1] = eff_pose_reference_nao[1] + 0.098 + 0.01;
        eff_pose_reference_nao[2] = eff_pose_reference_nao[2] - 0.1 + 0.01;        
        }
    ...
    double translation_threshold = 0.0001;
