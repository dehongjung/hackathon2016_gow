...
class KinectToNaoPublisher(object):
    ...
    def get_hip_coordinates(self):
        index_left = self.kinect_skeleton_frames.index("left_hip")
        index_right = self.kinect_skeleton_frames.index("right_hip")

        joint_left = self.kinect_frame_RPY.position[index_left]
        joint_right = self.kinect_frame_RPY.position[index_right]

        joint_position = Vector3()
        joint_position.x = (joint_left.x+joint_right.x)/2
        joint_position.y = (joint_left.y+joint_right.y)/2
        joint_position.z = (joint_left.z+joint_right.z)/2
    ...
    def manipulate_and_fill_msg_with_frame_information(self):
        ...
        if "left_hip" in self.kinect_frame_RPY.frame_name and "right_hip" in self.kinect_frame_RPY.frame_name:
            nao_hip = self.get_hip_coordinates()
            nao_joint = nao_hip
            nao_joint = self.apply_kalman_filter(self.kalman_hip, nao_joint, "Hip")
                
            nao_chain_msg = NaoChain()
        
            # Append effector name
            nao_chain_msg.effector = "Hip"
            
            # Append points
            nao_chain_msg.x = nao_joint.x
            nao_chain_msg.y = nao_joint.y
            nao_chain_msg.z = nao_joint.z

            self.nao_hip = nao_chain_msg
    ...
    def publish_nao_frames(self):
        ...
        """
        print 42
        self.hip_publisher.publish(self.nao_hip)
        print self.nao_hip
    ...
    def initialize_publishers(self):
        """
        """
        self.hip_publisher = rospy.Publisher('nao_hip', NaoChain)
    ...
    def initialize_messages(self):
        """
        """
        # Initialize the nao message (hip)
        self.nao_hip = NaoChain()
    ...
    def initialize_kalman_filter(self):
        """
        """
        # Initialize an object to use kalman filter for the hip
        self.kalman_hip = kalman_filter.KalmanFilter()    
